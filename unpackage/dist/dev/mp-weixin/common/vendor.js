...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "__UNI__B919F01",
    appName: "CloudVendor02",
    appVersion: "1.0.0",
    appVersionCode: "100",
...
function loadSDK(version2, key, libraries) {
  const libs = ["visualization", "tools", "geometry", ...libraries || []];
  return new Promise((resolve2) => {
    if (window.TMap) {
      resolve2(window.TMap);
      return;
    }
    ...
    script.src = `https://map.qq.com/api/gljs?v=${version2}&key=${key}&libraries=${libs.join(
      ","
...
var TMap$1 = /* @__PURE__ */ defineComponent({
  name: "tmap-map",
  props: {
    version: {
      type: String,
  ...
  setup(props) {
    ...
    onMounted(async () => {
      ...
      const center2 = new TMap.LatLng(props.center.lat, props.center.lng);
      if (el.value) {
    ...
    watch(
      ...
      ([center2, zoom, rotation, pitch]) => {
        if (mapIns) {
          mapIns.easeTo(
            {
              center: new TMap.LatLng(center2.lat, center2.lng),
              zoom,
              rotation,
              pitch
            },
...
function useEvent(overlay, attrs, emit2) {
  const events = [];
  const listeners = [];
  Object.keys(attrs).forEach((attr) => {
    if (attr.indexOf("on") === 0) {
      ...
      events.push(eventName);
      listeners.push(emit2.bind(null, eventName));
    }
  });
...
function useCleanUp(map2, id) {
  const overlay = map2.getLayer(id);
  if (overlay) {
    overlay.setMap(null);
  }
  const oldOverlayBorder = map2.getLayer(`${id}_border_line`);
  if (oldOverlayBorder) {
    oldOverlayBorder.setMap(null);
  }
  const overlayBorder = map2.getLayer(`${id}_border`);
  if (overlayBorder) {
...
var MultiMarker = /* @__PURE__ */ defineComponent({
  ...
  setup(props, { attrs, emit: emit2 }) {
    ...
    const originMap = toRaw(map2.value);
...
var MarkerCluster = /* @__PURE__ */ defineComponent({
...
function buildGeometries$1(geometries) {
  return geometries.map((v) => {
    return {
      ...v,
      paths: v.paths.map((p2) => new TMap.LatLng(p2.lat, p2.lng))
    };
  });
}
var MultiPolyline = /* @__PURE__ */ defineComponent({
...
var MultiPolygon = /* @__PURE__ */ defineComponent({
  ...
  setup(props, { attrs, emit: emit2 }) {
    ...
    useCleanUp(originMap, props.id);
    const polygon2 = new TMap.MultiPolygon({
      id: props.id,
      map: originMap,
      zIndex: props.zIndex,
    ...
    useEvent(polygon2, attrs, emit2);
    watch(
      () => props.zIndex,
      (zIndex) => {
        polygon2.setZIndex(zIndex);
      }
    );
    watch(
      () => props.styles,
      (styles) => {
        polygon2.setStyles(builtStyle$2(styles));
      }
    );
    watch(
      () => props.geometries,
      (geometries) => {
        ...
        if (toDelete.size > 0) {
          polygon2.remove([...toDelete]);
        }
        if (toAddOrModify.length > 0) {
          polygon2.updateGeometries(buildGeometries(geometries));
        }
      }
    );
    provide("geometry", polygon2);
    onUnmounted(() => {
...
var MultiLabel = /* @__PURE__ */ defineComponent({
  ...
  setup(props, { attrs, emit: emit2 }) {
    ...
    useEvent(labelInstance, attrs, emit2);
    watch(
      () => props.styles,
      (styles) => {
...
var MultiCircle = /* @__PURE__ */ defineComponent({
...
var InfoWindow = /* @__PURE__ */ defineComponent({
  ...
  setup(props, { emit: emit2 }) {
    ...
    const originMap = toRaw(map2.value);
    useCleanUp(originMap, props.id);
    const center2 = getLatLng$1(props.position);
...
var PolygonEditor = /* @__PURE__ */ defineComponent({
  ...
  setup(props, { emit: emit2 }) {
    ...
    editor.on("select", () => {
      emit2("select", editor.getSelectedList());
    });
    editor.on("draw_complete", (e2) => {
      emit2("update:modelValue", [...props.modelValue, e2]);
    });
    editor.on("adjust_complete", (e2) => {
      for (let i = props.modelValue.length - 1; i >= 0; i -= 1) {
        if (props.modelValue[i].id === e2.id) {
          Object.assign(props.modelValue[i], e2);
          emit2("update:modelValue", [...props.modelValue]);
          break;
        }
      }
    });
    editor.on("delete_complete", (e2) => {
      const removedIds = e2.map((v) => v.id);
      emit2(
        "update:modelValue",
    ...
    editor.on("union_complete", (e2) => {
      const activeOverlay = editor.getActiveOverlay();
      emit2("update:modelValue", [...activeOverlay.overlay.getGeometries(), e2]);
      emit2("select", editor.getSelectedList());
    });
    editor.on("split_fail", (e2) => {
      emit2("error", e2);
    });
    editor.on("union_fail", (e2) => {
      emit2("error", e2);
    });
    watch(
      () => props.actionMode,
...
var DOMOverlay = /* @__PURE__ */ defineComponent({
